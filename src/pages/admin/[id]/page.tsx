...
export const AdminPage: React.FC<{}> = () => {
  ...
  const stop = () => {
    ...
    QUERY_REFS.isGameOver({ gameId }).set(false);
    QUERY_REFS.activeQuote({ gameId }).remove()
      .then(() => toast(
        'Spel gestopt.',
        { type: 'success' }
      ))
  ...
  const start = () => {
    ...
    const quotesRef = QUERY_REFS.allQuotes({ gameId })
    quotesRef.once('value').then(snapshot => {
      participants?.forEach(({ id }) => QUERY_REFS.playerScore({ gameId, playerId: id }).set(0));
      const shuffledList = _.shuffle(snapshot.val());

      QUERY_REFS.activeQuote({ gameId }).set(shuffledList[0]);

      quotesRef.set(shuffledList)
        .then(() => {
          next('Spel gestart.');
        })
        .catch(() => toast(
  ...
  const showAnswer = () => {    
    ...
    QUERY_REFS.showAnswer({ gameId }).set(true)
      .then(() => {
        game?.persistAnswers();
        toast(
          ...
          return (
            <Flex px={2} width='100%' justifyContent='center' key={participant.id}>
              <ParticipantBar
                ...
                side={isCorrect && game?.quote?.showAnswer ? () => (
                  <Flex mr={1}>
