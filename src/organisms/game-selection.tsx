...
export const GameSelection: FC<{ destination: (_: string) => string }> = ({ destination }) => {
  const { data, isLoading, error } = useFirebaseData<AvailableGames>('availableGames');
  const games = Object.values(data?.val() ?? {});
  ...
  return (
    <PresentationTemplate>
      ...
      {data && games.map(game => (
        <GameOptionBlock key={game.uuid} {...game} onClick={() => showPasswordDialog(game.uuid)} />
      ))}
    </PresentationTemplate>
  );
}
